...
from trainer.selfsup.tasks import SelfSupFutureValueTask, build_future_value_batch, load_dataset_rows
...
def _require_torch():
    try:
        ...
        raise RuntimeError("PyTorch is required for train_future_value.py") from exc
...
def _materialize_rows_from_config(
    *,
    ...
    if not source_tokens:
        raise RuntimeError("future_value training requires data.dataset_path or data.sources")
...
def run_train_future_value(
    *,
    ...
    seed = int(seed_override if seed_override is not None else int(train_cfg.get("seed") or 3601))
    random.seed(seed)
    ...
    if not batch.features:
        raise RuntimeError("future_value dataset is empty")

    indices = list(range(len(batch.features)))
    ...
    class _DS(Dataset):
        ...
        def __getitem__(self, i: int) -> dict[str, Any]:
            j = self.idxs[i]
            return {"x": batch.features[j], "y": batch.targets[j]}

    def _collate(items: list[dict[str, Any]]) -> dict[str, Any]:
        return {
            "x": torch.tensor([x["x"] for x in items], dtype=torch.float32),
    ...
    task = SelfSupFutureValueTask(
        input_dim=len(batch.features[0]),
    ...
    if out_dir is not None:
        ...
        artifacts_root = str(output_cfg.get("artifacts_root") or "docs/artifacts/p36/future_value")
    ...
    def _evaluate(loader) -> tuple[float, float]:
        ...
        with torch.no_grad():
            for batch_t in loader:
                x = batch_t["x"].to(device)
                y = batch_t["y"].to(device)
                pred = task.forward(x)
                loss = task.loss(pred, y)
                total_loss += float(loss.item()) * int(y.numel())
                total_abs += float((pred - y).abs().sum().item())
                total_count += int(y.numel())
        return (total_loss / max(1, total_count), total_abs / max(1, total_count))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_t in train_loader:
            ...
            if global_step == 1 or global_step % max(1, log_every) == 0:
                _append_progress(
                    {
                        "schema": "p36_future_value_progress_v1",
                        "ts": _now_iso(),
                        "epoch": epoch,
                        "step": global_step,
        ...
        train_loss = float(epoch_loss / max(1, epoch_count))
        val_loss, val_mae = _evaluate(val_loader)
        row = {
            "epoch": epoch,
        ...
        _append_progress({"schema": "p36_future_value_progress_v1", "ts": _now_iso(), **row, "stage": "epoch_done"})
        ckpt = run_dir / f"future_value_epoch{epoch}.pt"
        torch.save({"model": task.model.state_dict(), "epoch": epoch, "seed": seed}, ckpt)
        if not quiet:
            print(
                "[p36-future] epoch={epoch} train_loss={train:.6f} val_loss={val:.6f} val_mae={mae:.6f}".format(
                    epoch=epoch,
                    train=train_loss,
                    val=val_loss,
                    mae=val_mae,
    ...
    with loss_curve_csv.open("w", encoding="utf-8", newline="") as fp:
        writer = csv.DictWriter(fp, fieldnames=["epoch", "train_loss", "val_loss", "val_mae", "step"])
        writer.writeheader()
        writer.writerows(loss_rows)

    final = loss_rows[-1] if loss_rows else {"val_loss": 0.0, "val_mae": 0.0, "train_loss": 0.0}
    summary = {
        "schema": "p36_future_value_summary_v1",
        "status": "ok",
        ...
        "seed": seed,
        "sample_count": len(batch.features),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="P36 self-supervised task: future value prediction.")
    p.add_argument("--config", default="configs/experiments/p36_selfsup_future_value.yaml")
