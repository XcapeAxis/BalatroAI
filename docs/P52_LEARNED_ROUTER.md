# P52 Learned Router / Meta-Controller v1

P52 upgrades the P48 rule-based hybrid controller with a lightweight learned router while keeping the original rule router as a safe fallback.
...
P48 proved that state-aware routing across controllers is useful and explainable, but its thresholds are hand-tuned. P52 adds a learnable meta-controller so routing can adapt to observed controller outcomes without discarding the P48 rule path.
...
Arena-facing policy ids used for P52 compare:
...
P52 v1 uses a pragmatic mixed labeling strategy:
...
- `docs/artifacts/p52/router_dataset/<run_id>/`
...
- `docs/artifacts/p52/router_train/<run_id>/`
...
P52 defaults to the P50 CUDA-first resolver path. On the validated local setup this resolves to `.venv_trainer_cuda` and `learner_device=cuda:0`.

## Safe Deployment Modes

P52 keeps the router deployment contract explicit:
...
- `docs/artifacts/p52/arena_ablation/<run_id>/`
...
P52 registers learned-router checkpoints through the P51 registry/state-machine path.
...
P52 campaigns are stage-aware and resumeable with the following stage ids:
...
P22 summaries for P52 now surface:
...
The dashboard now exposes P52-specific sections when artifacts are present:
