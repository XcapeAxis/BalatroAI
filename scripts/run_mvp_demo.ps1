...
if (-not $checkpointPath -or -not (Test-Path $checkpointPath)) {
  Write-Warning "[mvp-demo] no trained MVP checkpoint found. Run scripts\\bootstrap_mvp_demo.ps1 to build one."
} else {
  Write-Host ("[mvp-demo] latest checkpoint: " + $checkpointPath)
}
...
if ($Detach) {
  ...
  Write-Host ("[mvp-demo] detached pid=" + [string]$process.Id + " url=" + $url)
...
Write-Host ("[mvp-demo] python_env_name: " + [string]$resolver.selected.env_name)
Write-Host ("[mvp-demo] python_env_source: " + [string]$resolver.selection_reason)
