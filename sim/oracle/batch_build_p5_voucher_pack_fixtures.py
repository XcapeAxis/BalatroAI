...
from sim.oracle.generate_p5_voucher_pack_trace import generate_one_trace, load_supported_entries
from sim.oracle.p5_voucher_pack_classifier import build_and_write
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-build P5 voucher/pack fixtures and run directed oracle/sim diff.")
    parser.add_argument("--base-url", default="http://127.0.0.1:12346")
    parser.add_argument("--out-dir", default="sim/tests/fixtures_runtime/oracle_p5_voucher_pack_v1")
    parser.add_argument("--max-steps", type=int, default=260)
    parser.add_argument("--seed", default="AAAAAAA")
    parser.add_argument("--scope", choices=SCOPE_CHOICES, default="p5_voucher_pack_observed_core")
...
def _print_row_summary(row: dict[str, Any]) -> None:
    ...
    msg = (
        f"[P5] {row.get('target')} | {row.get('status')} | "
...
def _write_status_doc(project_root: Path, report: dict[str, Any], classifier_summary: dict[str, Any]) -> Path:
    doc_path = project_root / "docs" / "COVERAGE_P5_STATUS.md"
    ...
    lines: list[str] = []
    lines.append("# P5 Voucher/Pack Fixture Coverage Status")
    lines.append("")
    lines.append(f"- Generated at: `{report.get('generated_at')}`")
    lines.append(f"- Base URL: `{report.get('base_url')}`")
    lines.append(f"- Scope: `{report.get('scope')}`")
    lines.append(f"- Classifier total targets: **{classifier_summary.get('total', 0)}**")
...
def main() -> int:
    ...
    if int(args.workers) != 1:
        print(f"[P5] workers={args.workers} requested; forcing workers=1 for deterministic oracle alignment.")
    ...
    if not entries:
        print("ERROR: no P5 targets selected")
        return 2
    ...
    for entry in entries:
        ...
        for attempt in range(int(args.max_retries) + 1):
            ...
            print(f"[P5] target={target} attempt={attempt + 1}/{int(args.max_retries) + 1} generating trace...")
    ...
    report_path = out_dir / "report_p5.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    status_doc = _write_status_doc(project_root, report, classifier_summary)

    print(
        f"[P5] done total={len(rows)} pass={passed} diff_fail={diff_fail} "
    ...
    print(f"[P5] status_doc={status_doc}")
