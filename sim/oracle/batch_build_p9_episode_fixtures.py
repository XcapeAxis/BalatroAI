...
from sim.oracle.generate_p9_episode_trace import select_entries, generate_one_trace, load_supported_entries
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-build P9 episode fixtures and run directed oracle/sim diff.")
    parser.add_argument("--base-url", default="http://127.0.0.1:12346")
    parser.add_argument("--out-dir", default="sim/tests/fixtures_runtime/oracle_p9_episode_v1")
    parser.add_argument("--max-steps", type=int, default=800)
    parser.add_argument("--seed", default="AAAAAAA")
    parser.add_argument("--scope", choices=SCOPE_CHOICES, default="p9_episode_observed_core")
...
def _print_row_summary(row: dict[str, Any]) -> None:
    ...
    msg = (
        f"[P9-episode] {row.get('target')} | {row.get('status')} | "
...
def _write_status_docs(project_root: Path, report: dict[str, Any]) -> tuple[Path, Path]:
    ...
    episodes_path = project_root / "docs" / "COVERAGE_P9_EPISODES.md"
    ...
    status_lines.append("# P9 Episode Coverage Status")
    status_lines.append("")
    status_lines.append(f"- Generated at: `{report.get('generated_at')}`")
    status_lines.append(f"- Base URL: `{report.get('base_url')}`")
    status_lines.append(f"- Scope: `{report.get('scope')}`")
    status_lines.append("")
    status_lines.append("## Batch Result")
    ...
    status_lines.append("")
    status_lines.append("## Supported Boss Keys (fixture-selected)")
    for key, value in sorted(boss_counter.items()):
        status_lines.append(f"- `{key}`: {value}")
    status_lines.append("")
    status_lines.append("## Supported Tag Keys (fixture-selected)")
    for key, value in sorted(tag_counter.items()):
        status_lines.append(f"- `{key}`: {value}")
    status_lines.append("")
    status_lines.append("## Failing Status Counts")
    if fail_counter:
    ...
    episode_lines.append("# P9 Episode Fixture Targets")
    episode_lines.append("")
    for entry in load_supported_entries(project_root):
        episode_lines.append(
            f"- `{entry.get('target')}` [boss={entry.get('boss_key') or '-'} tag={entry.get('tag_key') or '-'}]"
        )
...
def main() -> int:
    ...
    if int(args.workers) != 1:
        print(f"[P9-episode] workers={args.workers} requested; forcing workers=1 for deterministic oracle alignment.")
    ...
    if not entries:
        print("ERROR: no P9 targets selected")
        return 2
    ...
    for entry in entries:
        ...
        row: dict[str, Any] = {
            "target": target,
            "category": str(entry.get("category") or "episode_loop"),
            "template": str(entry.get("template") or "p9_episode_loop_v1"),
        ...
        for attempt in range(int(args.max_retries) + 1):
            ...
            print(f"[P9-episode] target={target} attempt={attempt + 1}/{int(args.max_retries) + 1} generating trace...")
    ...
    report_path = out_dir / "report_p9.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    status_doc, episodes_doc = _write_status_docs(project_root, report)

    print(
        f"P9 episode summary: pass={report['passed']}/{report['total']} "
