...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze P8 RNG oracle/sim mismatch and emit categorized tables.")
...
def _classify(row: dict[str, Any], first_action: dict[str, Any], oracle0: dict[str, Any], sim0: dict[str, Any]) -> str:
    status = str(row.get("status") or "")
    if status == "pass":
        return "aligned"

    oracle_replay = oracle0.get("rng_replay") if isinstance(oracle0.get("rng_replay"), dict) else {}
    sim_replay = sim0.get("rng_replay") if isinstance(sim0.get("rng_replay"), dict) else {}
    if oracle_replay != sim_replay:
    ...
    if action_type not in {"PLAY", "DISCARD", "WAIT"}:
        return "non_rng_action"

    return "other_rng_mismatch"


def main() -> int:
    args = parse_args()
    fixtures_dir = Path(args.fixtures_dir)
    report_path = fixtures_dir / "report_p8_rng.json"
    ...
    for result in results:
        ...
        classification = _classify(result, action0, oracle0, sim0)
        ...
        rows.append(
            {
                ...
                "sim_delta": sim_delta,
                "delta_gap": delta_gap,
                "oracle_rng_outcomes": json.dumps((oracle0.get("rng_replay") or {}).get("outcomes") if isinstance(oracle0.get("rng_replay"), dict) else []),
                "sim_rng_outcomes": json.dumps((sim0.get("rng_replay") or {}).get("outcomes") if isinstance(sim0.get("rng_replay"), dict) else []),
    ...
    csv_path = fixtures_dir / "rng_mismatch_table_p8.csv"
    md_path = fixtures_dir / "rng_mismatch_table_p8.md"
    ...
    lines: list[str] = []
    lines.append("# P8 RNG Mismatch Table")
    lines.append("")
