...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze P2b oracle/sim mismatch and emit categorized tables.")
...
def _classify(row: dict[str, Any], first_action: dict[str, Any], expected: dict[str, Any]) -> str:
    ...
    planet_levels = int(expected.get("planet_levels") or 0)

    if "round.discards_left" in path or "round.hands_left" in path or group == "resource_sensitive":
        return "resource_mismatch"
    if "zones.played" in path or "zones.hand" in path or group == "order_selection":
        return "ordering_mismatch"
    if "joker" in path or group == "stacked":
        return "joker_mismatch"
    if planet_levels > 0 or group == "joker_planet" or "planet" in path:
        return "planet_mismatch"
...
def main() -> int:
    ...
    report_path = fixtures_dir / "report_p2b.json"
    ...
    for result in results:
        ...
        expected = oracle0.get("computed_expected") if isinstance(oracle0.get("computed_expected"), dict) else {}

        oracle_delta = _f(score_obs_oracle.get("delta"))
        sim_delta = _f(score_obs_sim.get("delta"))
        delta_gap = oracle_delta - sim_delta

        classification = _classify(result, action0, expected)
        ...
        rows.append(
            {
                ...
                "sim_delta": sim_delta,
                "delta_gap": delta_gap,
                "expected_core": _f(expected.get("score_core")),
                "expected_joker_delta": _f(expected.get("score_joker_delta")),
                "expected_total": _f(expected.get("score_total_expected") or expected.get("score")),
                "expected_planet_levels": int(expected.get("planet_levels") or 0),
    ...
    csv_path = fixtures_dir / "score_mismatch_table_p2b.csv"
    md_path = fixtures_dir / "score_mismatch_table_p2b.md"
    ...
    fieldnames = [
        ...
        "delta_gap",
        "expected_core",
        "expected_joker_delta",
        "expected_total",
        "expected_planet_levels",
        "classification",
        "first_diff_step",
        "first_diff_path",
        "dumped_oracle",
    ...
    lines: list[str] = []
    lines.append("# P2b Score Mismatch Table")
    lines.append("")
