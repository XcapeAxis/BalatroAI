...
from sim.oracle.generate_p4_consumable_trace import generate_one_trace, load_supported_entries
from sim.oracle.p4_consumable_classifier import build_and_write
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-build P4 consumable fixtures and run directed oracle/sim diff.")
    parser.add_argument("--base-url", default="http://127.0.0.1:12346")
    parser.add_argument("--out-dir", default="sim/tests/fixtures_runtime/oracle_p4_consumables_v1")
    parser.add_argument("--max-steps", type=int, default=220)
    parser.add_argument("--seed", default="AAAAAAA")
    parser.add_argument("--scope", choices=SCOPE_CHOICES, default="p4_consumable_observed_core")
...
def _print_row_summary(row: dict[str, Any]) -> None:
    ...
    msg = (
        f"[P4] {row.get('target')} | {row.get('status')} | "
...
def _write_status_doc(project_root: Path, report: dict[str, Any], classifier_summary: dict[str, Any]) -> Path:
    doc_path = project_root / "docs" / "COVERAGE_P4_STATUS.md"
    ...
    lines: list[str] = []
    lines.append("# P4 Consumable Fixture Coverage Status")
    lines.append("")
    lines.append(f"- Generated at: `{report.get('generated_at')}`")
    lines.append(f"- Base URL: `{report.get('base_url')}`")
    lines.append(f"- Scope: `{report.get('scope')}`")
    lines.append(f"- Classifier total consumables: **{classifier_summary.get('total', 0)}**")
...
def main() -> int:
    ...
    if int(args.workers) != 1:
        print(f"[P4] workers={args.workers} requested; forcing workers=1 for deterministic oracle alignment.")
    ...
    if not entries:
        print("ERROR: no P4 targets selected")
        return 2
    ...
    for entry in entries:
        ...
        set_type = str(entry.get("set_type") or "")
        consumable_key = str(entry.get("consumable_key") or "")
        ...
        for attempt in range(int(args.max_retries) + 1):
            ...
            print(f"[P4] target={target} attempt={attempt + 1}/{int(args.max_retries) + 1} generating trace...")
    ...
    report = {
        ...
        "classifier": {
            ...
            "supported_targets_path": _safe_relpath(Path(str(classifier_summary.get("supported_targets_path"))), project_root) if classifier_summary.get("supported_targets_path") else None,
        },
        "total": len(rows),
        "passed": passed,
        "diff_fail": diff_fail,
