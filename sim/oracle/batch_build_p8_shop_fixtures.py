...
from sim.oracle.generate_p8_shop_trace import load_supported_entries, select_entries, generate_one_trace
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-build P8 shop fixtures and run directed oracle/sim diff.")
    parser.add_argument("--base-url", default="http://127.0.0.1:12346")
    parser.add_argument("--out-dir", default="sim/tests/fixtures_runtime/oracle_p8_shop_v1")
    parser.add_argument("--max-steps", type=int, default=300)
    parser.add_argument("--seed", default="AAAAAAA")
    parser.add_argument("--scope", choices=SCOPE_CHOICES, default="p8_shop_observed_core")
...
def _print_row_summary(row: dict[str, Any]) -> None:
    ...
    msg = (
        f"[P8-shop] {row.get('target')} | {row.get('status')} | "
...
def _write_status_docs(project_root: Path, report: dict[str, Any]) -> tuple[Path, Path]:
    status_path = project_root / "docs" / "COVERAGE_P8_STATUS.md"
    shop_path = project_root / "docs" / "COVERAGE_P8_SHOP.md"
    ...
    status_lines.append("# P8 Shop Coverage Status")
    status_lines.append("")
    ...
    status_lines.append("")
    status_lines.append("## Category Counts")
    for cat, n in sorted(category_counter.items()):
        status_lines.append(f"- `{cat}`: {n}")
    status_lines.append("")
    status_lines.append("## Failing Categories")
    if fail_counter:
    ...
    shop_lines: list[str] = []
    shop_lines.append("# P8 Shop Fixture Targets")
    shop_lines.append("")
    for entry in load_supported_entries(project_root):
        shop_lines.append(
            f"- `{entry.get('target')}` [{entry.get('category')}] - {entry.get('description')}"
        )

    status_path.write_text("\n".join(status_lines) + "\n", encoding="utf-8")
    shop_path.write_text("\n".join(shop_lines) + "\n", encoding="utf-8")
    return status_path, shop_path
...
def main() -> int:
    ...
    if int(args.workers) != 1:
        print(f"[P8-shop] workers={args.workers} requested; forcing workers=1 for deterministic oracle alignment.")
    ...
    if not entries:
        print("ERROR: no P8 shop targets selected")
        return 2
    ...
    for entry in entries:
        ...
        for attempt in range(int(args.max_retries) + 1):
            ...
            print(f"[P8-shop] target={target} attempt={attempt + 1}/{int(args.max_retries) + 1} generating trace...")
    ...
    report_path = out_dir / "report_p8.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    status_doc, shop_doc = _write_status_docs(project_root, report)

    print(
        f"P8 shop summary: pass={report['passed']}/{report['total']} "
