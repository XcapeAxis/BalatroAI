...
from sim.oracle.p5_modifier_classifier import SUPPORTED_TEMPLATES, build_and_write
...
def _resolve_consumable_key(base_url: str, timeout_sec: float, candidates: list[str]) -> tuple[str | None, str | None]:
    keys = [str(x).strip().lower() for x in candidates if str(x).strip()]
...
def _extract_expected_context(params: dict[str, Any]) -> dict[str, Any] | None:
    ctx = params.get("expected_context")
...
def generate_one_trace(
    *,
    ...
    strategy = str(params.get("strategy") or "wait_only").strip().lower()
    cards_required = int(params.get("cards_required") or 0)
    expected_context = _extract_expected_context(params)
    allow_fallback_wait = bool(params.get("allow_fallback_wait", True))

    try:
        fallback_reason = None
        key_used: str | None = None
        use_meta: dict[str, Any] = {}

        if strategy.startswith("apply_consumable"):
            cands = params.get("consumable_candidates") if isinstance(params.get("consumable_candidates"), list) else []
            key_used, add_err = _resolve_consumable_key(base_url, timeout_sec, cands)
            if not key_used:
                if allow_fallback_wait:
                    strategy = "wait_only"
                    fallback_reason = f"add_consumable_failed:{add_err}"
                else:
        ...
        if strategy == "apply_consumable_then_play":
            hand_cards = extract_hand_cards(builder.state)
            if not hand_cards:
                if allow_fallback_wait:
                    strategy = "wait_only"
                    fallback_reason = (fallback_reason + "; " if fallback_reason else "") + "empty_hand_for_play"
                else:
                    raise RuntimeError("empty_hand_for_play")

        if strategy == "apply_consumable_then_play":
            builder.step("PLAY", indices=[0])
            if expected_context and builder.action_trace:
                builder.action_trace[-1]["expected_context"] = expected_context
        else:
...
def generate_many(
    *,
    ...
    for entry in entries:
        ...
        rows.append(row)
        print(f"[P5-gen] {target} | {row['status']} | steps={row['steps_used']} | {row['failure_reason'] or ''}")
    ...
    (out_dir / "report_generate_p5.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    return summary


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate P5 modifier fixtures (start snapshot + action trace) from template map.")
    parser.add_argument("--base-url", default="http://127.0.0.1:12346")
    parser.add_argument("--out-dir", default="sim/tests/fixtures_runtime/oracle_p5_modifiers_gen")
