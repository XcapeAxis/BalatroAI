...
from sim.oracle.generate_p2b_trace import P2B_TARGETS, generate_trace
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-build P2b joker interaction smoke fixtures and run directed diff.")
    parser.add_argument("--base-url", default="http://127.0.0.1:12346")
    parser.add_argument("--out-dir", default="sim/tests/fixtures_runtime/oracle_p2b_smoke/")
    parser.add_argument("--max-steps", type=int, default=120)
    parser.add_argument("--seed", default="AAAAAAA")
    parser.add_argument("--scope", choices=SCOPE_CHOICES, default="p2b_hand_score_observed_core")
...
def _parse_targets(targets_csv: str | None) -> list[str]:
    if not targets_csv:
        return sorted(P2B_TARGETS.keys())
    out: list[str] = []
    for token in str(targets_csv).split(","):
        name = token.strip()
        if not name:
            continue
        if name not in P2B_TARGETS:
...
def _print_row_summary(row: dict[str, Any]) -> None:
    ...
    msg = (
        f"[P2b] {row.get('target')} | {row.get('status')} | "
...
def main() -> int:
    ...
    if int(args.workers) != 1:
        print(f"[P2b] workers={args.workers} requested; forcing workers=1 for deterministic oracle alignment.")
    ...
    for target in selected_targets:
        ...
        row: dict[str, Any] = {
            "target": target,
            "group": str((P2B_TARGETS.get(target) or {}).get("group") or ""),
            "status": "fail",
        ...
        for attempt in range(int(args.max_retries) + 1):
            ...
            print(f"[P2b] target={target} attempt={attempt + 1}/{int(args.max_retries) + 1} generating trace...")
    ...
    report_path = out_dir / "report_p2b.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    print(
        f"[P2b] done total={len(rows)} pass={passed} diff_fail={diff_fail} "
        f"oracle_fail={oracle_fail} gen_fail={gen_fail} skipped={skipped}"
    )
    print(f"[P2b] report={report_path}")
