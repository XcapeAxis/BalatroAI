...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze P5 voucher/pack oracle/sim mismatch and emit categorized tables.")
...
def _classify(row: dict[str, Any], first_action: dict[str, Any]) -> str:
    ...
    if action_type not in {"PLAY", "BUY", "PACK", "USE", "WAIT"}:
        return "non_play_action"

    path = str(row.get("first_diff_path") or "")
    if "vouchers" in path or "used_vouchers" in path:
        return "voucher_state_mismatch"
    if "packs" in path or "shop" in path:
        return "pack_state_mismatch"
    if "zones.hand" in path:
        return "hand_state_mismatch"
...
def main() -> int:
    ...
    report_path = fixtures_dir / "report_p5.json"
    ...
    for result in results:
        ...
        rows.append(
            {
                "target": target,
                "set_type": str(result.get("set_type") or ""),
    ...
    csv_path = fixtures_dir / "score_mismatch_table_p5_voucher_pack.csv"
    md_path = fixtures_dir / "score_mismatch_table_p5_voucher_pack.md"
    ...
    lines: list[str] = []
    lines.append("# P5 Voucher/Pack Mismatch Table")
    lines.append("")
