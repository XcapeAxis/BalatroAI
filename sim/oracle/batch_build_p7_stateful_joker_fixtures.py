...
from sim.oracle.generate_p7_stateful_joker_trace import generate_one_trace, load_supported_entries
from sim.oracle.p7_stateful_joker_classifier import build_and_write
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-build P7 stateful joker fixtures and run directed oracle/sim diff.")
    parser.add_argument("--base-url", default="http://127.0.0.1:12346")
    parser.add_argument("--out-dir", default="sim/tests/fixtures_runtime/oracle_p7_stateful_v1")
    parser.add_argument("--max-steps", type=int, default=260)
    parser.add_argument("--seed", default="AAAAAAA")
    parser.add_argument("--scope", choices=SCOPE_CHOICES, default="p7_stateful_observed_core")
...
def _print_row_summary(row: dict[str, Any]) -> None:
    ...
    msg = (
        f"[P7] {row.get('target')} | {row.get('status')} | "
...
def _write_status_doc(project_root: Path, report: dict[str, Any], classifier_summary: dict[str, Any]) -> Path:
    doc_path = project_root / "docs" / "COVERAGE_P7_STATUS.md"
    ...
    lines: list[str] = []
    lines.append("# P7 Stateful Joker Fixture Coverage Status")
    lines.append("")
...
def main() -> int:
    ...
    if int(args.workers) != 1:
        print(f"[P7] workers={args.workers} requested; forcing workers=1 for deterministic oracle alignment.")
    ...
    if not entries:
        print("ERROR: no P7 targets selected")
        return 2
    ...
    for entry in entries:
        ...
        for attempt in range(int(args.max_retries) + 1):
            ...
            print(f"[P7] target={target} attempt={attempt + 1}/{int(args.max_retries) + 1} generating trace...")
