...
from sim.oracle.generate_p8_rng_trace import select_entries, generate_one_trace
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-build P8 RNG fixtures and run directed oracle/sim diff.")
    parser.add_argument("--base-url", default="http://127.0.0.1:12346")
    parser.add_argument("--out-dir", default="sim/tests/fixtures_runtime/oracle_p8_rng_v1")
    parser.add_argument("--max-steps", type=int, default=260)
    parser.add_argument("--seed", default="AAAAAAA")
    parser.add_argument("--scope", choices=SCOPE_CHOICES, default="p8_rng_observed_core")
...
def _print_row_summary(row: dict[str, Any]) -> None:
    ...
    msg = (
        f"[P8-rng] {row.get('target')} | {row.get('status')} | "
...
def _write_status_docs(project_root: Path, report: dict[str, Any]) -> tuple[Path, Path]:
    ...
    rng_path = project_root / "docs" / "COVERAGE_P8_RNG.md"
    ...
    status_lines.append("# P8 RNG Replay Coverage Status")
    status_lines.append("")
    ...
    rng_lines: list[str] = []
    rng_lines.append("# P8 RNG Fixture Targets")
    rng_lines.append("")
    for entry in select_entries(None, None):
        rng_lines.append(f"- `{entry.get('target')}` [{entry.get('category')}] - {entry.get('description')}")

    status_path.write_text("\n".join(status_lines) + "\n", encoding="utf-8")
    rng_path.write_text("\n".join(rng_lines) + "\n", encoding="utf-8")
    return status_path, rng_path
...
def main() -> int:
    ...
    if int(args.workers) != 1:
        print(f"[P8-rng] workers={args.workers} requested; forcing workers=1 for deterministic oracle alignment.")
    ...
    if not entries:
        print("ERROR: no P8 rng targets selected")
        return 2
    ...
    for entry in entries:
        ...
        for attempt in range(int(args.max_retries) + 1):
            ...
            print(f"[P8-rng] target={target} attempt={attempt + 1}/{int(args.max_retries) + 1} generating trace...")
    ...
    report_path = out_dir / "report_p8_rng.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    status_doc, rng_doc = _write_status_docs(project_root, report)

    print(
        f"P8 rng summary: pass={report['passed']}/{report['total']} "
